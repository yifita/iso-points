...
class PointsRasterizationSettings:
    ...
    Attributes:
        ...
        image_size: (int) image size in pixels, supporting atm only square image
        points_per_pixel (int): rasterize maximum of K points per pixel
        bin_size (int): used for faster forward-pass
        max_points_per_bin (int): see pytorch3d point rasterizer
...
class SurfaceSplatting(PointsRasterizer):
    ...
    def filter_renderable(self, point_clouds: PointClouds3D, point_clouds_filter=None, **kwargs):
        raster_settings = kwargs.get("raster_settings", self.raster_settings)
        cameras = kwargs.get('cameras', self.cameras)
        if point_clouds.isempty():
            return point_clouds, torch.full((point_clouds.num_points_per_cloud().sum(),), True, dtype=torch.bool, device=point_clouds.device)
    ...
    def _compute_isotropic_Vrk(self, pointclouds, refresh=True, **kwargs):
        ...
        The larger h_k is, the larger the splat
        Args:
    ...
    def _empty_fragments(self, batch_size, **kwargs):
        ...
        """
        raster_settings = kwargs.get("raster_settings", self.raster_settings)
        device = kwargs.get('device', 'cuda:%d' % torch.cuda.current_device())
        ...
        qvalue_map = torch.full(
            (batch_size, S, S, P), -1.0, dtype=torch.float, device=device)
        frag_scaler = qvalue_map
        occ_map = torch.full((batch_size, S, S), 0,
                             dtype=torch.float, device=device)
        return PointFragments(idx=idx, zbuf=zbuf, qvalue=qvalue_map, scaler=frag_scaler, occupancy=occ_map)
    ...
    def forward(self, point_clouds, point_clouds_filter=None, **kwargs) -> PointFragments:
        ...
        if len(point_clouds) == 1 and len(cameras) > 1:
