...
class Model(nn.Module):
    ...
    def render(self, p_world=None, cameras=None, lights=None) -> torch.Tensor:
        ...
        # render
        rgba = self.renderer(colored_pointclouds, self.points_filter, cameras)
        self.points_filter.activation = self.points_filter.activation[:1]
        self.points_filter.visibility = self.points_filter.visibility.any(
            dim=0, keepdim=True)
        self.points_filter.inmask = self.points_filter.inmask[:1]
...
class Generator(BaseGenerator):
    ...
    def generate_mesh(self, *args, **kwargs):
        ...
        for b in range(len(points)):
            ...
            mesh = ms.Mesh(vertex_matrix=points[b].detach().cpu().numpy(),v_normals_matrix=normals[b].detach().cpu().numpy())
            ms.add_mesh(mesh)
            ms.surface_reconstruction_screened_poisson()
            reconst = ms.current_mesh()
            mesh = trimesh.Trimesh(
                vertices=reconst.vertex_matrix(), faces=reconst.face_matrix(), process=False)
            meshes.append(mesh)
