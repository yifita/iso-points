...
def reduce_mask_padded(values: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    ...
    dtype = value_packed.dtype
    value_padded = packed_to_padded(
        value_packed.float(), first_idx, num_true_in_batch.max().item()).to(dtype=dtype)
    return value_padded
...
def get_visible_points(point_clouds, cameras, depth_merge_threshold=0.05, return_mask=False):
    """ Returns packed visibility """
    from DSS.core.rasterizer import SurfaceSplatting
    from DSS.core.rasterizer import PointsRasterizationSettings as SplatterSetting
    from DSS.core.cloud import PointCloudsFilters
    splatter = SurfaceSplatting(raster_settings=SplatterSetting(
        depth_merging_threshold=depth_merge_threshold, image_size=256, cutoff_threshold=1.0, backface_culling=True))
    pcl_filter = PointCloudsFilters(device=point_clouds.device)
    splatter(point_clouds, cameras=cameras, point_clouds_filter=pcl_filter)
    filtered_pcl = pcl_filter.filter_with(point_clouds, ('visibility', ))
    if return_mask:
        return filtered_pcl, pcl_filter.visibility
    return filtered_pcl
